...
include 'database.php';
         $name = $_POST['email']; 
            ...
            if($my_id_array['status'] == 2)
                    {
                        ...
                        echo "<script>window.location = 'loginAEmp.html';</script>";
                    }
            } else {
            ...
            $result = mysqli_query($conn, "SELECT * FROM `msd_register_comp_employee_table` WHERE `emp_email` = '".$name."' AND `emp_password` = '".$password."'");
                $my_id_array=mysqli_fetch_assoc($result);
                $fullname =$my_id_array['emp_name'];
            ...
            { 
                 ...
                 $_SESSION["MGRID"] = $userid;
                 $_SESSION["EMPID"] = $userid;
                 //$_SESSION["AGENTID"] = $userid;
                 echo get_client_ip();
                    echo "<script>window.location = 'employeeDashboard';</script>";
                    exit();
            }
            else
            {
