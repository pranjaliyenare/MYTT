...
<?php
    ...
    if ($_SESSION['ROLE'] == "employee") {
        ...
        // Total Incentive
        $inc_query = mysqli_query($conn, "SELECT MONTH(`date`) AS MONTH, SUM(`emp_incentive`) as amt FROM `msd_customer_plan_table` WHERE `emp_id` = '".$_SESSION['USERID']."' AND `status` !=2 GROUP BY MONTH;");
        while ($rowinc = $inc_query->fetch_assoc()) {
            if ($rowinc["MONTH"] == $month) {
                $month_inc = $rowinc["amt"];
            }
        }
        ...
        // Yearly Profit
        $profit_query = mysqli_query($conn, "SELECT SUM(`cust_profit_amount`) AS amount, YEAR(PROFIT.`date`) AS year FROM `msd_profit_table` AS PROFIT INNER JOIN `msd_register_customer_table` ON `register_id` = `userid` WHERE `reference_id` = '".$_SESSION['USERID']."' AND `status` != 2");
        while ($row = $profit_query->fetch_assoc()) {
            $profit_amt = $row["amount"];
        }

        // Yearly Withdraw
        $query = mysqli_query($conn, "SELECT SUM(`amount`) AS amount, YEAR(WITHDRAW.`date`) AS year FROM `msd_transaction_withdraw_table` AS WITHDRAW INNER JOIN `msd_register_customer_table` ON `register_id` = `user_id` WHERE `reference_id` = '".$_SESSION['USERID']."' AND `approve_status` = 'approved' AND`status` != 2");
        while ($rowWith = $query->fetch_assoc()) {
            $withdraw_amt = $rowWith["amount"];
        }
            
        //Monthly deposit
        $month_dep_query = mysqli_query($conn, "SELECT MONTH(`pay_date`) AS MONTH, SUM(amount) as amt FROM `msd_xway_pay_response_table` INNER JOIN `msd_register_customer_table` ON `register_id` = `userid` WHERE `reference_id` = '".$_SESSION['USERID']."' AND `status` != 'failed' AND `pay_status` !=2 GROUP BY MONTH(`pay_date`)");
                 ...
                 <div class="app-main__outer">
                    <div class="app-main__inner">       
                        ...
                        <?php   
                            ...
                            $query1 = mysqli_query($conn, "SELECT COUNT(*) AS cnt FROM `msd_register_comp_agent_table` WHERE `refer_emp_mgr_id` = '".$_SESSION['USERID']."' AND `status` != 2");
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 col-xl-4">
                                <div class="card mb-3 widget-content bg-grow-early">
                                    <div class="widget-content-wrapper text-white">
                                        <div class="widget-content-left">
                                              <div class="widget-heading">Total Incentive</div>
                                            <div class="widget-subheading">Monthly Incentive</div>
                                        </div>
                                        <div class="widget-content-right">
                                            <div class="widget-numbers text-white"><span><?php echo $month_inc; ?></span></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 col-xl-4">
                                <div class="card mb-3 widget-content" style="background-image: linear-gradient(to top, #844c5a 0%, #71132a 100%) !important;">
                                    <div class="widget-content-wrapper text-white">
                                        <div class="widget-content-left">
                                            <div class="widget-heading">Yearly Total Deposit</div>
                            ...
                            <div class="col-md-6 col-xl-4">
                                <div class="card mb-3 widget-content" style="background-image: linear-gradient(to top, #ea6c97 0%, #e91e63 100%) !important;">
                                    <div class="widget-content-wrapper text-white">
                                        <div class="widget-content-left">
                                            <div class="widget-heading">Yearly Total Profit</div>
                            ...
                            <div class="col-md-6 col-xl-4">
                                <div class="card mb-3 widget-content" style="background-image: linear-gradient(to top, #ffeb3b 0%, #ff9800 100%) !important;">
                                    <div class="widget-content-wrapper text-white">
                                        <div class="widget-content-left">
                                            <div class="widget-heading">Yearly Total Withdraw</div>
                            ...
                            <div class="col-md-12">
                                <div class="main-card mb-3 card">
                                    ...
                                    <?php
                                        ...
                                        $projects = array();
                                        $records = mysqli_query($conn, "SELECT `register_id` FROM `msd_register_customer_table` WHERE `reference_id` = '".$_SESSION['USERID']."' AND `register_approved_status` = 'approved' AND `register_status` != 2");
                                       ...
                                       //$queryTable = "SELECT cust.`register_id` AS id, concat(cust.`register_fname`, ' ',cust.`register_lname`) AS name, cust.`register_profit_perc` AS cust_perc, cust.`register_agent_profit_perc` AS agent_perc, PAYRES.amount AS deposit, T1.withdraw, T2.cust_profit, T2.agent_profit FROM `msd_register_customer_table` AS cust LEFT JOIN `msd_xway_pay_response_table` PAYRES ON  PAYRES.userid = cust.`register_id` AND PAYRES.`pay_status` != 2 LEFT JOIN (SELECT `register_id`,`user_id`, `amount` AS withdraw FROM `msd_transaction_withdraw_table` INNER JOIN `msd_register_customer_table` ON `user_id` = `register_id` WHERE `approve_status` = 'approved' AND `reference_id` = '".$_SESSION['USERID']."') AS T1 ON T1.user_id = cust.`register_id` LEFT JOIN (SELECT PROFIT.cust_profit_amount AS cust_profit, PROFIT.`agent_profit_amount` AS agent_profit, userid, status FROM `msd_profit_table` PROFIT LEFT JOIN `msd_register_customer_table` CUST ON PROFIT.userid =register_id WHERE CUST.`reference_id` = '".$_SESSION['USERID']."') AS T2 ON T2.userid = PAYRES.userid AND T2.`status` != 2 WHERE cust.`reference_id` = '".$_SESSION['USERID']."' AND cust.register_approved_status = 'approved'"; 
                                        ...
                                        echo '<table class="align-middle mb-0 table table-borderless table-striped table-hover">
                                            ...
                                            <tr>
                                                ...
                                                <th class="text-center">Agent Profit</th>
                                            ...
                                            foreach ($projects as $project)
                                            {
                                                $queryCust = mysqli_query($conn, "SET sql_mode = 'NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'");     
                                                    ...
                                                    // WITHDRAWAL DETAILS
                                                    $queryWithd = mysqli_query($conn,"SELECT COUNT(*) AS payout_cnt, `user_id` id, SUM(`amount`) with_amt, SUM(`principal_amount`) AS princ_amt FROM `msd_transaction_withdraw_table` WITHD WHERE WITHD.`user_id` = '". $project["register_id"]."' AND WITHD.`approve_status` = 'approved' AND WITHD.`status` != 2");
                                                    ...
                                                    // DEPOSIT DETAILS
                                                    $queryDep = mysqli_query($conn,"SELECT  `userid` id, SUM(`amount`) inv_amt, pay_date FROM `msd_xway_pay_response_table` WHERE `status` != 'failed' AND `userid` = '". $project["register_id"]."' AND `pay_status` != 2");
                                                    ...
                                                    // PERCENTAGE DETAILS
                                                    $queryPerc = mysqli_query($conn,"SELECT `customer_id` AS id, `customer_perc` AS prof_perc FROM `msd_transaction_role_perc_table` WHERE `customer_id` = '". $project["register_id"]."' AND `status` != 2");
                                                    ...
                                                    // PROFIT TABLE
                                                    $queryProfit = mysqli_query($conn,"SELECT PROFIT.`userid` id, SUM(PROFIT.`cust_profit_amount`) profit_amt, SUM(PROFIT.`agent_profit_amount`) AS agent_amount FROM `msd_profit_table` PROFIT WHERE PROFIT.`userid` = '". $project["register_id"]."' AND PROFIT.`status` != 2");
