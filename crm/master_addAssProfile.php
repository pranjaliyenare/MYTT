...
<?php
 ...
 if(isset($_POST['submit'])) {       
       ...
       if(isset($_POST['submit']) && !empty($_FILES['file']['name'])) { 
        ...
        $table="msd_register_comp_assistant_table";
        $field =array("`ass_id`, `reference_id`, `ass_name`, `ass_mobile`, `ass_email`, `ass_password`, `ass_re_password`, `ass_address`, `ass_gender`, `ass_image`, `ass_role_type`");
        $data =array("'".$_POST['id_name']."', '".$_POST['reference']."', '". $_POST['name']."', '". $_POST['mobile']."','". strtolower($_POST['email'])."', '". $_POST['password']."', '". $_POST['confirm_password']."', '". $_POST['address']."', '". $_POST['radio2']."', '". $_FILES['file']['name']."', 'assistant'");
        $field_values= implode(',',$field);
        $data_values=implode(',',$data);
        
       echo $query="INSERT INTO". " ".$table." (".$field_values. ") VALUES(".$data_values.")";    
        $result = $conn->query($query);

        $query1="INSERT INTO `msd_login_table`(`userid`,`username`, `password`, `type`) VALUES ('".$_POST['id_name']."','".strtolower($_POST['email'])."', '".$_POST['password']."', 'assistant')";
        $result1 = $conn->query($query1);
         if($result)
         {
            echo "<script>alert('Your Form Submitted Successfully');</script>";
            //echo "<script>window.location = 'master_displayAssistant?role=".base64_encode($role)."';</script>";
         }
         else
         {
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="main-card mb-3 card">
                                            ...
                                            <form class="" action="master_addAssProfile" method="POST" enctype="multipart/form-data" id="uploadForm">
                                                <div class="position-relative row form-group"><label for="name_id" class="col-sm-2 col-form-label">ID</label>
                                                    <div class="col-sm-10">
                                                        <?php                                                
                                                        $query = mysqli_query($conn, "SELECT lpad(MAX(`id`)+1,4,'0')id FROM `msd_register_comp_assistant_table` ");
                                                        ...
                                                        <div class="position-relative row form-group"><label for="image_id" class="col-sm-2 col-form-label">Image</label>
                                                            <div class="col-sm-10">
                                                                <form method="post" enctype="multipart/form-data" id="uploadForm">
                                                                    ...
                                                                    <input type="button" onclick="window.location = 'master_displayAssistant?role=<?php echo base64_encode($role) ?>'" class="btn btn-secondary" name="close_name" value="Close"/> 
                                                                </form>
                                                            </div>
                                                        </div>
                                                    </div>
