...
<?php include 'header.php';
      ...
      if(base64_decode($_GET['type']) == "edit") {
            ...
            $query = mysqli_query($conn, "SELECT * FROM `msd_register_comp_assistant_table` WHERE ass_id = '".$id."' and `status` != 2");
            $my_array=mysqli_fetch_assoc($query);

            $my_id=$my_array["ass_id"];
            $reference_id=$my_array["reference_id"];
            $name=$my_array["ass_name"];
            $mobile=$my_array["ass_mobile"];
            $email=$my_array["ass_email"];
            $pass=$my_array["ass_password"];
            $address=$my_array["ass_address"];
            $gender=$my_array["ass_gender"];
            $image=$my_array["ass_image"];
            
      }
    //Delete Record
    if(base64_decode($_GET['type']) == "delete") {
            mysqli_query($conn,"UPDATE `msd_register_comp_assistant_table` SET `status`= 2 WHERE `ass_id` = '".$id."'");
            mysqli_query($conn,"UPDATE `msd_login_table` SET `status` = 2 WHERE `userid`='".$id."'");
            echo "<script>window.location = 'master_displayAssistant';</script>";
    }
?>
...
include 'database.php';
    ...
    if(isset($_POST['submit'])) {
        ...
        if($_FILES['file']['name']){
            ...
            mysqli_query($conn,"UPDATE `msd_register_comp_assistant_table` SET `reference_id`='".$_SESSION["ADMINID"]."',`ass_name`='". $_POST['name']."', `ass_mobile`='". $_POST['mobile']."', `ass_email`='". strtolower($_POST['email'])."', `ass_address`='". $_POST['address']."', `ass_gender`='". $_POST['radio2']."',`ass_image`='". $profile."', `status`=1 WHERE `ass_id`= '".$id."'");
...
<head>
    ...
    <title>Edit Admin Assistant Profile.</title>
                                            ...
                                            <form class="" method="POST" name="uploadFormname" enctype="multipart/form-data" id="uploadForm">
                                                ...
                                                <div class="position-relative row form-group"><label for="image_id" class="col-sm-2 col-form-label">Image</label>
                                                    ...
                                                    <form method="post" name="uploadFormname" enctype="multipart/form-data" id="uploadForm">
                                                        ...
                                                        <input type="button" onclick="window.location = 'master_displayAssistant';" class="btn btn-secondary" name="close_name" value="Close"/>
                                                    </form>                                                    
                                                    </div>
                                                </div>
                                            </form>
