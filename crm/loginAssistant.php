<?php
        include 'database.php';
            ...
            if($my_id_array['status'] == 2)
            {
                ...
                echo "<script>window.location = 'loginAdminAssi.html';</script>";
            }
            } else {
            ...
            $result = mysqli_query($conn, "SELECT * FROM `msd_register_comp_assistant_table` WHERE `ass_email` = '".$name."' AND `ass_password` = '".$password."'");
                $my_id_array=mysqli_fetch_assoc($result);
                $fullname =$my_id_array['ass_name'];
            ...
            { 
                 ...
                 $_SESSION["ADMINID"] = $refid;
                    echo get_client_ip();
                    echo "<script>window.location = 'assistantDashboard';</script>";
                    exit();
            }
            else
            {
