...
include 'database.php';
         $name = $_POST['email']; 
            ...
            $result = mysqli_query($conn, "SELECT * FROM `msd_register_comp_manager_table` WHERE `mgr_email` = '".$name."' AND `mgr_password` = '".$password."'");
                $my_id_array=mysqli_fetch_assoc($result);
                $fullname =$my_id_array['mgr_name'];
            ...
            { 
                ...
                echo get_client_ip();
                echo "<script>window.location = 'managerDashboard';</script>";
                    exit();
            }
            else
            {
