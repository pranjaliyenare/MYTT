                        ...
                        <div class="table-responsive">
                            <p style='margin-bottom: 0;' align="left"> 
                                <?php
                                    ...
                                    $sql = "SELECT * FROM `msd_register_comp_agent_table` WHERE `agent_id` ='".$_SESSION['USERID']."' AND `status` != 2";
                                    ...
                                    $query = "SELECT id, name, dt, refno, deposit, withdrawals, profit, planid FROM (SELECT register_id id, concat(register_id , ' - ', register_fname,' ', register_lname) AS name, PROFIT.date AS dt, 'profit' AS refno, 0.00 AS deposit, 0.00 AS withdrawals, PROFIT.amt AS profit, PROFIT.plan_id AS planid FROM (SELECT *, `agent_profit_amount` amt FROM `msd_profit_table` WHERE `admin_id` = '".$_SESSION['USERID']."' UNION SELECT *, `agent_profit_amount2` amt FROM `msd_profit_table` WHERE `agent_id2` = '".$_SESSION['USERID']."' UNION SELECT *, `agent_profit_amount3` AS amt FROM `msd_profit_table` WHERE `agent_id3` = '".$_SESSION['USERID']."' UNION SELECT *, `agent_profit_amount4` AS amt FROM `msd_profit_table` WHERE `agent_id4` = '".$_SESSION['USERID']."' UNION SELECT *, `agent_profit_amount5` AS amt FROM `msd_profit_table` WHERE `agent_id5` = '".$_SESSION['USERID']."' UNION SELECT *, `agent_profit_amount6` AS amt FROM `msd_profit_table` WHERE `agent_id6` = '".$_SESSION['USERID']."') PROFIT INNER JOIN `msd_register_customer_table` register ON register_id = userid WHERE register.register_status != 2 UNION SELECT '0' AS id, 'Wallet' AS name, WITHDRAW.approve_date AS dt, 'withdraw' AS refno, 0.00 AS deposit, WITHDRAW.amount AS withdrawals, 0.00 AS profit, WITHDRAW.plan_id AS planid FROM `msd_transaction_withdraw_table` WITHDRAW WHERE WITHDRAW.`approve_status` = 'approved' AND WITHDRAW.`user_id` = '".$_SESSION['USERID']."' AND status != 2) tbl ORDER BY dt";
                                    $bal = 0.00;
                                    $profit = 0.00;
                                    $tot_dep = 0.00;
                                    $tot_profit = 0.00;
